...
__global__ void computeRHSSharedKernel(float *d_r, float *d_ru, float *d_rv, float *d_e,
    ...
    // Adding 2 in each dimension for halo points
    ...
    // Halo points - left column
    if (tx == 0 && i > 0 && j < Ny) {
        int global_idx = idx(i-1, j, Nx, Ny);
    ...
    // Halo points - right column
    if (tx == blockDimX-1 && i+1 < Nx && j < Ny) {
        int global_idx = idx(i+1, j, Nx, Ny);
    ...
    if (i < Nx && j < Ny) {
        ...
        // Advection terms using shared memory
